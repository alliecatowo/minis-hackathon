...
def _build_framework_scope_metadata(
    ...
    if visible_stable_framework_ids and visible_project_preference_ids:
        rationale = (
            "Scoped signals lead but durable temporal frameworks are preserved in visibility."
        )
    elif visible_stable_framework_ids:
    ...
    for signal in signals:
        ...
        if policy.context in {"hotfix", "incident"}:
            if "shipping_speed" in dimensions:
                context_boost += 0.2
                reasons.append(
                    f"{policy.context} pressure favors restoring service over broadening scope"
                )
...
def _build_framework_signals(
    ...
    scored: list[tuple[int, float, int, bool, bool, ReviewPredictionFrameworkSignalV1]] = []
    for raw in frameworks:
        ...
        raw_confidence = _coerce_confidence(raw.get("confidence"), default=0.5)
        ...
        confidence = _coerce_confidence(raw_confidence + temporal_boost + scope_match_boost)
    ...
    if not ordered:
        return [], None
    visible: list[tuple[int, float, int, bool, bool, ReviewPredictionFrameworkSignalV1]] = ordered[
    ...
    if visible:
        ...
        if stable_candidates and not has_stable:
            replacement = stable_candidates[0]
            visible[-1] = replacement
            visible = sorted(
                visible,
