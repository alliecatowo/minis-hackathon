...
2. Always check setup first:
   ...
   If the JSON says `"available": false`, stop and show the setup instructions. Required env: `MINIS_TOKEN` or `MINIS_AUTH_TOKEN`. Optional env: `MINIS_API_BASE` or `MINIS_BACKEND_URL`.
...
6. If the user specifically asks for MCP configuration, show the `MINIS_BACKEND_URL` and `MINIS_AUTH_TOKEN` env shape from the `remote-check` output and point them to `mcp-server/README.md`. Do not invent a token or claim the MCP server is authenticated unless setup check passed.
